#!/usr/bin/env python3
"""
Test legacy SCP (without SFTP subsystem)
"""
...
def test_legacy_scp(config):
    """Test legacy SCP with -O flag to avoid SFTP"""
    ...
    # Build SCP command with legacy mode
    scp_cmd = []
    ...
    scp_cmd.append("scp")
    scp_cmd.extend([
        "-O",  # Use legacy SCP protocol (no SFTP)
        "-v",  # Verbose
        "-P", nas_port,
    ...
    print(f"🔍 Legacy SCP Command: {' '.join(['sshpass', '-p', '***'] + scp_cmd[3:])}")
    print("🚀 Executing legacy SCP transfer...")
...
if __name__ == "__main__":
    print("🔍 Legacy SCP Transfer Test")
    print("=" * 40)
    ...
    try:
        if connect_vpn(config):
            success = test_legacy_scp(config)
            if success:
                print("🎉 Legacy SCP transfer successful!")
            else:
                print("❌ Legacy SCP transfer failed")
        else:
